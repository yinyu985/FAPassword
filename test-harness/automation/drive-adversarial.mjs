...
const BASE = (process.env.FAPASSWORD_BASE || "http://127.0.0.1:8799") + "/adversarial";
const results = [];
const ok = (n,c,d) => { results.push({n,c}); console.log(`${c?"PASS":"FAIL"} ${n}${c?"":" -> "+d}`); };

const ctx = await chromium.launchPersistentContext("/tmp/fapassword-adv-"+Date.now(), {
...
// mixed page: dropdown only on login fields, not the search box
{
  ...
  if (searchSel) {
    await page.locator(searchSel).first().click().catch(()=>{});
    await page.waitForTimeout(300);
  ...
  if (await pwd.count()) {
    await pwd.click(); await page.waitForTimeout(300);
